...
export function KineticMatrix({
    ...
    const anomaliesRef = useRef<LocalizedAnomaly[]>([]);
        ...
        for (let c = 0; c < cols; c++) {
            for (let r = 0; r < rows; r++) {
                ...
                nodes.push({
                    ...
                    row: r,
                    radius: 1.4,
                    label: `ANX${((c * 17 + r * 31) % 256).toString(16).padStart(2, '0').toUpperCase()}`,
                    tension: 0,
    ...
    useEffect(() => {
        ...
        const render = (now: number) => {
            ...
            for (let s = shockwaves.length - 1; s >= 0; s--) {
                ...
                sw.radius += 500 * dt; // Faster shockwave expansion
                sw.power *= Math.pow(0.08, dt); // Smoother, slightly slower decay
                if (sw.radius > sw.maxRadius || sw.power < 0.01) {
                    shockwaves.splice(s, 1);
                }
            }
            
            // Render and update localized Anomalies (Tap Effects)
            for (let t = anomalies.length - 1; t >= 0; t--) {
                const anomaly = anomalies[t];
                const elapsed = now - anomaly.createdAt;
                
                // Complete cycle within ~3.5s
                if (elapsed > 3500) {
                    anomalies.splice(t, 1);
                    continue;
                }
                
                let strength = 1.0;
                if (elapsed > 2000) {
                    // Smooth close over the remaining 1500ms
                    const progress = (elapsed - 2000) / 1500;
                    const ease = progress * progress * (3 - 2 * progress);
                    strength = 1 - ease; // Fade out strength
                }
                
                // Draw localized anomaly core
                ctx.beginPath();
                ctx.arc(anomaly.x, anomaly.y, 40 * strength + Math.sin(now * 0.005) * 5 * strength, 0, Math.PI * 2);
                ctx.fillStyle = `rgba(${accentGlow}, ${0.1 * strength})`;
                ctx.fill();
                
                ctx.beginPath();
                ctx.arc(anomaly.x, anomaly.y, 80 * strength - Math.cos(now * 0.004) * 10 * strength, 0, Math.PI * 2);
                ctx.strokeStyle = `rgba(${accentGlow}, ${0.2 * strength})`;
                ctx.lineWidth = 2 * strength;
                ctx.stroke();
                
                // We'll apply the physical pull directly in the nodes loop
            }

            // 2. Physics Step (Hooke's Spring-Mass Lattice)
            const SPRING_K = 28; // Cinematic fluid tension
            const DAMPING = 0.88; // Lower damping for prolonged smooth motion
            const VELOCITY_SCALE = 65; // Balanced, premium speed

            for (let i = 0; i < nodes.length; i++) {
                const n = nodes[i];
                n.pulsePhase += dt * 2.5; // Smooth cinematic pulse
                ...
                if (dist < pointer.radius && dist > 0) {
                    // Smooth cubic easing for fluid interaction instead of linear
                    const ratio = 1 - dist / pointer.radius;
                    const smoothRatio = ratio * ratio * (3 - 2 * ratio); 
                    const force = smoothRatio * (1800 + mouseSpeed * 150 + (pointer.isDown ? 2500 : 0));
                    const angle = Math.atan2(dy, dx);

                    n.vx -= Math.cos(angle) * force * dt;
                    n.vy -= Math.sin(angle) * force * dt;
                    n.tension = Math.min(1, n.tension + smoothRatio * 0.6);
                }
                ...
                for (let s = 0; s < shockwaves.length; s++) {
                    ...
                    if (delta < 65) {
                        const ratio = 1 - delta / 65;
                        const smoothRatio = ratio * ratio * (3 - 2 * ratio);
                        const force = smoothRatio * sw.power * 3500;
                ...
                // Apply anomalies pull
                for (let a = 0; a < anomalies.length; a++) {
                    const anomaly = anomalies[a];
                    const elapsed = now - anomaly.createdAt;
                    let strength = 1.0;
                    if (elapsed > 2000) {
                        const progress = (elapsed - 2000) / 1500;
                        strength = 1 - (progress * progress * (3 - 2 * progress));
                    }
                    
                    const effectRadius = 350 * strength;
                    const adx = anomaly.x - n.x;
                    const ady = anomaly.y - n.y;
                    const adist = Math.sqrt(adx * adx + ady * ady);
                    
                    if (adist < effectRadius && adist > 0) {
                        const ratio = 1 - adist / effectRadius;
                        const smoothRatio = ratio * ratio * (3 - 2 * ratio);
                        // Pull nodes towards anomaly center
                        const force = smoothRatio * 1800 * strength;
                        const angle = Math.atan2(ady, adx);
                        n.vx += Math.cos(angle) * force * dt;
                        n.vy += Math.sin(angle) * force * dt;
                        n.tension = Math.max(n.tension, smoothRatio * strength * 0.8);
                    }
                }

                // CONTINUOUS AMBIENT DRIFT
                // Keeps the grid organically moving forever so it never freezes
                const time = now * 0.0003;
                const driftX = Math.sin(time + n.col * 0.15) * Math.cos(time * 0.7 + n.row * 0.15) * 16;
                const driftY = Math.cos(time * 0.8 + n.col * 0.15) * Math.sin(time + n.row * 0.15) * 16;

                const targetX = n.baseX + driftX;
                const targetY = n.baseY + driftY;

                const hx = targetX - n.x;
                const hy = targetY - n.y;
                n.vx += hx * SPRING_K * dt;
                n.vy += hy * SPRING_K * dt;

                // Frame-independent damping for ultra-smooth rendering on any monitor refresh rate
                const fpsScale = dt * 60;
                n.vx *= Math.pow(DAMPING, fpsScale);
                n.vy *= Math.pow(DAMPING, fpsScale);
                
                n.x += n.vx * dt * VELOCITY_SCALE;
                n.y += n.vy * dt * VELOCITY_SCALE;

                n.tension = Math.max(0, n.tension - dt * 1.2);
            }

            // 3. Spawn Random Synaptic Traveling Pulses
            if (Math.random() < 0.4 && nodes.length > 0 && pulses.length < 50) {
    ...
    return (
        ...
        >
            ...
            <div className="relative z-20 flex h-full w-full flex-col justify-between p-6 md:p-10 pointer-events-none">
                ...
                <header className="flex w-full items-center justify-between font-mono text-[11px] text-neutral-500 dark:text-neutral-400 pointer-events-auto">
                    <div className="flex items-center gap-2">
                        ...
                        >
                            ...
                            <span className="hidden sm:inline font-mono text-[10px]">ANX PULSE</span>
                        </button>
                ...
                {/* Center Hero Stencil Typography with ANX Matrix theme */}
                <main className="pointer-events-none flex flex-col items-center justify-center text-center">
                    <span className="text-xs uppercase tracking-[0.3em] text-[#7C3AED] font-bold mb-3 bg-black/40 px-3 py-1 rounded-full border border-[#7C3AED]/30 backdrop-blur-sm">
                        ANX DIGITAL AGENCY NETWORK
                    </span>
                    <h1 className="font-mono text-4xl sm:text-7xl md:text-8xl font-black tracking-tighter uppercase text-neutral-900 dark:text-white drop-shadow-2xl">
                        {title}
                    </h1>
                </main>
